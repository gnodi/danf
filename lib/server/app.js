...
function AppBuilder(serverConfiguration, clientConfiguration, serverContext, clientContext) {
    ...
    this._availableServers = {
        http: {sharable: true, proxy: true},
        socket: {sharable: true, proxy: true},
        command: {sharable: false, proxy: false}
    };
    ...
    if (context.cluster && cluster.isMaster) {
        ...
        for (var i = 0; i < context.cluster.length; i++) {
            ...
            for (var j = 0; j < chunkContext.workers; j++) {
                workers[i] = forkWorker(chunkContext);
            }

            if (chunkContext.proxy) {
                var getRandomWorkerIndex = function(ip, workers) {
                        var flatIp = '';
                ...
                var server = net.createServer(
                        {pauseOnConnect: true},
                        function(connection) {
                            var index = getRandomWorkerIndex(connection.remoteAddress, num_processes),
