...
AppBuilder.prototype.buildServer = function(done) {
    ...
    if (cluster.isMaster && context.cluster.active) {
        ...
        cluster.on('listening', function(worker, address) {
            ...
            if (processingWorkers === workersNumber) {
                setTimeout(
                    function() {
                        logListening('', '', self._serverContext.env, context.cluster.workers, cpuCount);
                    },
                    100
                );
            }
        ...
        app.create = function(callback) {
            ...
            framework.build(self._serverConfiguration, context, function() {
        ...
        app.logListening = function(server, port) {
            logListening(server, port, app.context.env, context.cluster.workers, cpuCount);
        };
        ...
        app.listenHttp = function(end) {
            ...
            server.listen(port, function() {
                if (cluster.worker) {
                    ...
                    context.worker = workerId;
                    logger.log(
                        '<<green>>Worker <<bold>>{0}<</bold>> processing <<bold>>HTTP requests<</bold>> on port <<bold>>{1}<</bold>>...'.format(
                            workerId,
                            port
                        ),
                        1
        ...
        app.listenCmd = function(end) {
            ...
            server.listen(port, function() {
                if (cluster.worker) {
                    ...
                    context.worker = workerId;
                    logger.log(
                        '<<green>>Worker <<bold>>{0}<</bold>> processing <<bold>>commands<</bold>> on port <<bold>>{1}<</bold>>...'.format(
                            workerId,
                            port
                        ),
                        1
...
AppBuilder.prototype.buildClient = function(done) {
    ...
    var optimize = context.debug ? 'none' : 'uglify2',
        relativizePath = function(targetPath) {
            ...
            if (null == relativizedPath) {
                relativizedPath = fs.realpathSync(path.join(self._paths.common, targetPath));
            }

            return path.relative(self._paths.common, relativizedPath).replace(/\.js$/, '');
        },
        buildConfigurations = [],
        resolvedPaths = self.resolvePaths(
            path.dirname(this._paths.appAbsolute),
            path.join(this._paths.common, this._paths.danfRelative)
        );
    ;
    ...
    for (var key in build.paths) {
        if (undefined === paths[key]) {
            paths[key] = 0 === build.paths[key].indexOf('lib')
                ? relativizePath(path.join(this._paths.danfRelative, build.paths[key]))
    ...
    for (var key in build.map) {
        key = 0 === key.indexOf('lib')
            ? relativizePath(path.join(this._paths.danfRelative, key))
            : key
        ;
        ...
        for (var embeddedKey in build.map[key]) {
            embeddedKey = 0 === embeddedKey.indexOf('lib')
                ? relativizePath(path.join(this._paths.danfRelative, embeddedKey))
                : embeddedKey
            ;

            map[key][embeddedKey] = 0 === build.map[key][embeddedKey].indexOf('lib')
                ? relativizePath(path.join(this._paths.danfRelative, build.map[key][embeddedKey]))
    ...
    var appMainRelativePath = this._paths.appMainRelative;
    ...
    if ('object' === typeof this._clientConfiguration) {
        ...
        var appEntryBuildFilename = path.join(path.dirname(this._paths.clientBuilt), 'app-config-entry.js');
            ...
            'module.exports = {0};'.format(toSource(this._clientConfiguration).replace(
                ...
                function(match, moduleFullPath) {
                    return 'require(\'{0}\')'.format(
                        moduleFullPath.replace(this._paths.common + '/', '')
                    );
                }
            ))
        );

        appMainRelativePath = relativizePath(appEntryBuildFilename);
    }
    ...
    buildConfiguration = {
        map: {
            '*': {
                _app: this._paths.appMainRelative === '' ? '/' : this._paths.appMainRelative.replace(/\\/g, '/')
            }
        },
 ...
 */
AppBuilder.prototype.buildClientJs = function(buildConfigurations, rebuild, commandName, callback) {
    var self = this,
 ...
 */
AppBuilder.prototype.buildSideConfiguration = function(modulePath, danfPath, sideName) {
