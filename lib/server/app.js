...
function createApplication(serverConfiguration, clientConfiguration, serverContext, clientContext, callback) {
    ...
    if ('auto' === clientConfiguration) {
        clientConfiguration = this.buildClientConfiguration(clientConfiguration);
    }
    ...
    if ('string' === typeof serverConfiguration) {
        ...
        if ('auto' === serverConfiguration) {
            serverConfiguration = this.buildServerConfiguration(serverConfiguration);
        } else {
    ...
    if (cluster.isMaster && 'test' !== context.environment) {
        ...
        var optimize = clientContext.debug ? 'none' : 'uglify2',
            ...
            buildConfigurations = [],
            resolvedPaths = this.resolvePaths(path.dirname(appMainAbsolutePath), path.join(commonPath, danfRelativePath));
        ;
        ...
        this.buildClientJs(buildConfigurations, function () {
 ...
 */
createApplication.prototype.buildClientJs = function(buildConfigurations, callback) {
    ...
    if (buildConfiguration) {
        ...
        if (isGeneration) {
            ...
            this.buildClientJs(buildConfigurations, callback);
        } else {
 ...
 */
createApplication.prototype.buildClientConfiguration = function(modulePath, danfPath) {
    var configuration = this.buildSpecificConfiguration(modulePath, danfPath, 'client');
 ...
 * @return {object} The configuration.
 */
createApplication.prototype.buildServerConfiguration = function(modulePath, danfPath) {
    var configuration = this.buildSpecificConfiguration(modulePath, danfPath, 'server');

    return configuration;
}

/**
 * Build specific side configuration.
 *
 * @param {string} modulePath The path of the module.
 * @param {string} danfPath The danf path.
 * @param {string} specificName The specific name.
 * @return {object} The configuration.
 */
createApplication.prototype.buildSpecificConfiguration = function(modulePath, danfPath, specificName) {
    ...
    try {
        commonConfiguration = this.buildConfiguration(
            path.join(modulePath, '/config/common')
        );
    } catch (error) {
    ...
    try {
        ...
        commonConfiguration.config.classes = utils.merge(
            ...
            this.buildConfiguration(
                path.join(modulePath, '/lib/common'),
                ''
            ),
            true
        );
    ...
    // Build specific side configuration.
    try {
        specificConfiguration = this.buildConfiguration(
            path.join(modulePath, '/config/{0}'.format(specificName))
        );
    } catch (error) {
        // Handle not existing specific config folder.
        if ('ENOENT' !== error.code) {
            throw error;
        }
    }

    // Build specific side classes configuration.
    try {
        if (undefined === specificConfiguration.config) {
            specificConfiguration.config = {};
        }

        specificConfiguration.config.classes = utils.merge(
            specificConfiguration.config.classes || {},
            this.buildConfiguration(
                path.join(modulePath, '/lib/{0}'.format(specificName)),
                ''
            ),
            true
        );
    } catch (error) {
        // Handle not existing specific lib folder.
        if ('ENOENT' !== error.code) {
            throw error;
        }
    }
            ...
            for (var i = 0; i < modulesDirectory.length; i++) {
                ...
                try {
                    var stats = fs.statSync(
                        path.join(modulesDirectoryPath, 'danf-{0}.js'.format(specificName))
                    );

                    if (stats.isFile()) {
                        dependenciesConfiguration[directory] = this.buildSpecificConfiguration(
    ...
    // Merge common with specific configurations, specific overwriting common.
 ...
 * @param {string} flatteningPath The optional flattening path if the configuration must be flattened.
 *
 * @return {object} The configuration.
 */
createApplication.prototype.buildConfiguration = function(currentPath, flatteningPath) {
    var configuration = {},
    ...
    for (var i = 0; i < directoryItems.length; i++) {
        ...
        key = flatteningPath ? '{0}.{1}'.format(flatteningPath, key) : key;

        // Handle directory.
        if (stats.isDirectory()) {
            if (undefined === flatteningPath) {
                var keys = key.split('.');
                ...
                for (var j = 0; j < keys.length; j++) {
                    ...
                    if (keys.length - 1 === j) {
                        config[subKey] = utils.merge(
                            config[subKey] || {},
                            this.buildConfiguration(fullPath),
                            true
                        );
                    } else {
                        if (undefined === config[subKey]) {
                            config[subKey] = {};
                        }
        ...
        // Handle file.
        } else if (stats.isFile() && '.js' === path.extname(key)) {
            key = path.basename(key, '.js');

            var content = require(fullPath);

            if ('object' === typeof content) {
                config[key] = utils.merge(
                    config[key] || {},
                    content,
                    true
                );
