...
AppBuilder.prototype.mergeDynamicParameters = function(parameters, sideName) {
    ...
    try {
        ...
        parameters = utils.merge(parameters, require(file));
    } catch (error) {
            ...
            try {
                ...
                if (stats.isFile()) {
                    ...
                    if (dependencyConfiguration) {
                        var dependencyName = this.interpretPath(directory.replace(/^danf-/, ''));

                        dependenciesConfiguration[dependencyName] = dependencyConfiguration;
                    }
                }
            } catch (error) {
 ...
 * @api protected
 */
 AppBuilder.prototype.buildDepencies = function(rootPath, callback) {
    var danfModules = {},
        isDanfModule = function(modulePath) {
    ...
    rpt(
        ...
        function (node, kidName) {
            var packageId = node.package._id;

            if (undefined === danfModules[packageId]) {
                danfModules[packageId] = isDanfModule(node.realpath);
            }

            return danfModules[packageId];
        },
        function (er, data) {
                            ...
                            while (currentNode) {
                                for (var i = 0; i < currentNode.children.length; i++) {
                                    ...
                                    if (child.package.name === name) {
                                        ...
                                        if (childDependency) {
                                            dependency.dependencies[child.package.name] = childDependency;
                                        }
            ...
            var dependencies = findDependencies(data).dependencies,
                dependedUpon = {}
            ;
            ...
            var flattenDependencies = function(contextDependencies, namespace) {
                    ...
                    for (var name in contextDependencies) {
                        var contextDependency = utils.clone(dependencies[name]),
                            ...
                            dependencyReferenceName = ':{0}'.format(contextDependency.name)
                        ;

                        aliases[name] = dependencyReferenceName;
                        ...
                        for (var dependencyName in flattenedDependencies) {
                            ...
                            dependedUpon[flattenedDependency.slice(1)] = true;
            ...
            for (var name in dependencies) {
                ...
                if (dependency.rooted) {
                    if (!dependedUpon[dependency.name]) {
 ...
 AppBuilder.prototype.displayDependencies = function(dependencies) {
    ...
    for (var name in dependencies) {
        ...
        if ('object' === typeof dependency) {
            ...
            for (var i = 0; i < dependency.versions.length; i++) {
                var version = dependency.versions[i];
            ...
            if (null != dependency.conflict) {
                if (dependency.conflict <= versionDiff.premajor) {
                    color = 'red';
                } else {
                    color = 'yellow';
                }
