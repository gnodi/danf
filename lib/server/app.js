...
function createApplication(serverConfiguration, clientConfiguration, serverContext, clientContext, callback) {
    ...
    if (cluster.isMaster && 'test' !== context.environment) {
        ...
        if ('object' === typeof clientConfiguration) {
            ...
            var appEntryBuildFilename = path.join(path.dirname(appMainRelativePath), 'app-config-generated-entry.js');
        ...
        self.buildClientJs(buildConfigurations, !context.nobuild, function () {
 ...
 */
createApplication.prototype.buildClientJs = function(buildConfigurations, rebuild, callback) {
    var self = this,
    ...
    if (buildConfiguration) {
        ...
        if (!rebuild) {
            try {
                ...
                self.buildClientJs(buildConfigurations, rebuild, callback);
...
createApplication.prototype.buildSideConfiguration = function(modulePath, danfPath, sideName) {
    ...
    // Build side side configuration.
    try {
        sideConfiguration = this.buildConfiguration(
            path.join(modulePath, '/config/{0}'.format(sideName))
        );
    } catch (error) {
    ...
    // Build side side classes configuration.
    try {
        ...
        sideConfiguration.config.classes = utils.merge(
            ...
            this.buildConfiguration(
                path.join(modulePath, '/lib/{0}'.format(sideName)),
                ''
            ),
            true
        );
    ...
    for (var i = 0; i < directoryItems.length; i++) {
        ...
        if (stats.isDirectory()) {
            if (undefined === flatteningPath) {
                var keys = key.replace(/\.\./, '-_-').split('.');

                for (var j = 0; j < keys.length; j++) {
                    var subKey = keys[j].replace(/-_-/g, '.');
        ...
        // Handle file.
            ...
            try {
                var content = undefined === flatteningPath
                        ? require(fullPath)
                        : 'require(\'{0}\')'.format(fullPath)
                ;
            } catch (error) {
                throw new Error('Interpretation of "{0}" failed: {1}'.format(
                    fullPath,
                    error.message
                ))
            }
