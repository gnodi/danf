 ...
 *
 * @param {string} rootPath The path of the module.
 * @param {string} danfPath The danf path.
...
createApplication.prototype.buildSpecificConfiguration = function(modulePath, danfPath, specificName) {
    var commonConfiguration = {},
        ...
        dependenciesPath = path.join(sourcePath, 'node_modules')
    ;

    try {
        commonConfiguration = this.buildConfiguration(
            path.join(rootPath, '/config/common')
        );
    } catch (error) {
    ...
    try {
        specificConfiguration = this.buildConfiguration(
            path.join(rootPath, '/config/{0}'.format(specificName))
        );
    } catch (error) {
    ...
    for (var i = 0; i < directoryItems.length; i++) {
        ...
        if (stat.isDirectory()) {
            ...
            for (var j = 0; j < keys.length; j++) {
                ...
                if (keys.length - 1 === j) {
                    config[subKey] = this.buildConfiguration(fullPath);
                } else {
                    config[subKey] = {};
