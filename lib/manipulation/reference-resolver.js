...
define(function(require) {
    ...
    ReferenceResolver.prototype.resolve = function(source, type, context, inText) {
        ...
        if (!concatenate) {
            ...
            if (resolvedReferences && Array.isArray(resolvedReferences.values) && 1 === resolvedReferences.values.length) {
                return resolvedReferences.values[0];
            }
        ...
        // This is a concatenation.
                ...
                if (1 === (i + offset) % 2 && '' !== splitSource[i]) {
                    ...
                    if (-1 === splitSource[i].indexOf(' ')) {
                        if (
                            ...
                            for (var j = 0; j < independentResolvedReferences.values.length; j++) {
                                independentReferences[i].push(independentResolvedReferences.values[j][0]);
                            }
                        } else {
            ...
            if (0 === mergedReferences.length) {
                mergedReferences = resolvedReferences.values;
            // Merge independent and context references.
            } else if (0 !== contextReferences.length) {
                var contextReferences = resolvedReferences.values,
                    referencesProduct = []
                ;
    ...
    var resolveBranchReferences = function(branch, context, simple) {
        var references = { values: [], isArray: false },
            branchesReferences = { values: [], isArray: false },
            branchKeyReferences = {}
        ;
            ...
            if (simple) {
                if (undefined !== context) {
                    for (var i = 0; i < branch.references.length; i++) {
                        references.values[branch.references[i]] = context;
                    ...
                    for (var key in context) {
                        if (Array.isArray(context[key])) {
                            for (var i = 0; i < context[key].length; i++) {
                                ...
                                references.values.push(branchReferences);
                            }
                        } else {
                            var branchReferences = [];
                    ...
                    if (context[key]) {
                        ...
                        if (undefined !== referenceValue) {
                            ...
                            if (!branchKeyReferences[key]) {
                                branchKeyReferences[key] = { values: [], isArray: false };
                            }
        ...
        if (0 === branchesReferences.values.length) {
            for (var key in branchKeyReferences) {
                branchesReferences.values.push(branchKeyReferences[key]);
                branchesReferences.isArray = (branchesReferences.isArray || branchKeyReferences[key].isArray);
            }

            if (0 !== references.values.length && 0 !== branchesReferences.values.length) {
                var mergedReferences = { values: [], isArray: false };

                references.isArray = (references.isArray || branchesReferences.isArray);

                for (var i = 0; i < references.values.length; i++) {
                    var referenceLength = references.values[i].length;

                    for (var j = 0; j < branchesReferences.values[i].values.length; j++) {
                        var refValues = branchesReferences.values[i].values[j],
                            mergedReference = []
                        ;

                        for (var k = 0; k < referenceLength; k++) {
                            if (undefined !== references.values[i][k]) {
                                mergedReference[k] = references.values[i][k];
                            } else if (undefined !== refValues[k]) {
                                mergedReference[k] = refValues[k];
                            }
                        }

                        mergedReferences.values.push(mergedReference);
                    }
                }
    ...
    var mergeBranchReferences = function(references1, references2) {
        //if (undefined === references2 || undefined === references1) {
        //    return;
        //}

        var references = { values: [], isArray: (references1.isArray || references2.isArray) };

        if (0 === references1.values.length) {
            for (var i = 0; i < references2.values.length; i++) {
                var refValues = references2.values[i];

                if (refValues && refValues.values) {
                    for (var j = 0; j < refValues.values.length; j++) {
                        references.values.push(refValues.values[j]);
                    }
                } else {
                    references.values.push(refValues);
                }
            }
        } else if (0 === references2.values.length) {
            for (var i = 0; i < references1.values.length; i++) {
                var refValues = references1.values[i];
            ...
            for (var i = 0; i < references1.values.length; i++) {
                var refValues1 = references1.values[i];

                for (var j = 0; j < references2.values.length; j++) {
                    var refValues2 = references2.values[j];

                    if (refValues1.values && refValues2.values) {
                        for (var l = 0; l < refValues1.values.length; l++) {
                            for (var m = 0; m < refValues2.values.length; m++) {
                                var referenceLength = Math.max(refValues1.values[l].length, refValues2.values[m].length)
                                    mergedReference = []
                                ;

                                for (var k = 0; k < referenceLength; k++) {
                                    if (undefined !== refValues1.values[l][k]) {
                                        mergedReference[k] = refValues1.values[l][k];
                                    } else if (undefined !== refValues2.values[m][k]) {
                                        mergedReference[k] = refValues2.values[m][k];
                                    }
                                }

                                references.values.push(mergedReference);
                            }
                        }
                    } else if (refValues1.values) {
                        for (var l = 0; l < refValues1.values.length; l++) {
                            var referenceLength = Math.max(refValues1.values[l].length, refValues2.length)
                                mergedReference = []
                            ;
                    ...
                    } else if (refValues2.values) {
                        for (var l = 0; l < refValues2.values.length; l++) {
                            var referenceLength = Math.max(refValues2.values[l].length, refValues1.length)
                                mergedReference = []
                            ;

                            for (var k = 0; k < referenceLength; k++) {
                                if (undefined !== refValues2.values[l][k]) {
                                    mergedReference[k] = refValues2.values[l][k];
