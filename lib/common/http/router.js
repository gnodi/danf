...
Router.prototype.follow = function(url, method, parameters, headers) {
    ...
    var meta = this.parse(url);

    for (var key in meta.parameters) {
        parameters[key] = meta.parameters[key];
    }

    this.find(url, method, true).follow(
        parameters,
        headers,
        meta
    );
...
Router.prototype.parse = function(url) {
    ...
    if (parsedUrl.search) {
        // Aggregate path.
        path += '?{0}'.format(parsedUrl.search);
