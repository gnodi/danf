...
Object.defineProperty(Route.prototype, 'host', {
    set: function(host) {
        ...
        this._port = parseInt(hostParts[1]);
    },
    get: function() {
        return this._port
...
Route.prototype.resolve = function(parameters) {
    ...
    var url = this._path.replace(/(\/):([^\/?]+)|(=):([^&]+)(?:&|$)/g, function(match, $1, $2, $3, $4) {
        var parameter = $2 || $4;
    ...
    for (var name in parameters) {
        if (!usedParameters[name]) {
            queryStringParameters.push(
                null == parameters[name] || '' == parameters[name]
                    ? '{0}'.format(name)
                    : '{0}={1}'.format(name, parameters[name])
            );
        }
    }

    if (0 !== queryStringParameters.length) {
        if (-1 === url.indexOf('?')) {
            url += '?';
        } else {
            url += '&';
        ...
        url += queryStringParameters.join('&');
    }
