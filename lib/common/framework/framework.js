 ...
 * @param {function} done The callback to call when the framework has built all its features.
 * @api public
 */
Framework.prototype.build = function(configuration, context, done) {
    var self = this,
    ...
    for (var i = this._initializers.length - 1; i >= 0; i--) {
        process = (function(i, callback) {
            var initializer = self._initializers[i];

            return function() {
                if (initializer.process) {
                    initializer.process(self, parameters, danfConfig, configuration, callback);
                } else {
                    callback();
                }
            };
