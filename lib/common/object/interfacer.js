    ...
    for (var i = 0; i < properties.length; i++) {
        ...
        if ('__' !== propertyName.slice(0, 2)) {
            ...
            var interface_ = this._interfacesContainer.get(interfaceName),
                descriptor = Object.getPropertyDescriptor(object, propertyName)
            ;

            if (
                (
                    null == descriptor || 'function' !== typeof descriptor.value
                ) &&
                (
        ...
        proxy[methodName] = function() {
            ...
            if (methodDefinition['returns']) {
                try {
                    ...
                    if (error.instead) {
                        error.message = 'The method "{0}"{1} defined by the interface "{2}" returns {3}; {4} given instead.'.format(
                            methodName,
...
var wrapProperty = function(object, interfaceName, proxy, propertyName) {
    ...
    if (null === propertyDescriptor || (undefined === propertyDescriptor.get && undefined === propertyDescriptor.value && undefined === propertyDescriptor.set)) {
        // Handle case where the property has no descriptor.
    ...
    if (!hasGetter) {
        ...
        propertyDescriptor.get = function() {
            ...
            try {
                ...
                if (error.instead) {
                    error.message = 'The getter "{0}"{1} defined by the interface "{2}" returns {3}; {4} given instead.'.format(
                        propertyName,
                        this.__metadata && this.__metadata.id ? ' of "{0}"'.format(this.__metadata.id) : '',
                        interfaceName,
                        error.expected,
    ...
    if (!hasSetter) {
        ...
        propertyDescriptor.set = function(value) {
            try {
                ...
                if (error.instead) {
                    error.message = 'The setter "{0}"{1} defined by the interface "{2}" takes {3}; {4} given instead.'.format(
                        propertyName,
                        this.__metadata && this.__metadata.id ? ' of "{0}"'.format(this.__metadata.id) : '',
                        interfaceName,
                        error.expected,
...
var checkMethodArguments = function (methodName, methodInterfaceName, methodArgTypes, args) {
    ...
    for (var i = 0; i < argsNumber; i++) {
        try {
            ...
            if (error.instead) {
                error.message = 'The method "{0}"{1} defined by the interface "{2}" takes {3} as argument {4}; {5} given instead.'.format(
                    methodName,
                    this.__metadata && this.__metadata.id ? ' of "{0}"'.format(this.__metadata.id) : '',
                    methodInterfaceName,
                    error.expected,
