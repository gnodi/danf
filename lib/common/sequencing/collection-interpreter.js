...
CollectionInterpreter.defineImplementedInterfaces(['danf:sequencing.collectionInterpreter']);

CollectionInterpreter.defineDependency('_referencesResolver', 'danf:sequencing.referencesResolver');
CollectionInterpreter.defineDependency('_flowDriver', 'danf:manipulation.flowDriver');
CollectionInterpreter.defineDependency('_logger', 'danf:sequencing.logger');
...
/**
 * @interface {danf:sequencing.collectionInterpreter}
 */
...
CollectionInterpreter.prototype.interpret = function(
    ...
    if ('string' === typeof input) {
        input = self._referencesResolver.resolve(input, flow.currentStream);
    ...
    args.push(asynchronousCollection.wrapIterator(function(parameters) {
        ...
        var resolvedArguments = operationArguments;

        if (parameters.memo) {
            resolvedArguments = self._referencesResolver.resolveSpecific(resolvedArguments, '~', parameters.memo);
        }
        resolvedArguments = self._referencesResolver.resolveSpecific(resolvedArguments, '@', retrieveContext());
        resolvedArguments = self._referencesResolver.resolveSpecific(resolvedArguments, '@', value);
        resolvedArguments = self._referencesResolver.resolveSpecific(resolvedArguments, '$');
        resolvedArguments = self._referencesResolver.resolveSpecific(resolvedArguments, '!');
