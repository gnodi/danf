...
Embedded.prototype.interpretEmbedded = function(embedded, sequenceId) {
    ...
    return function(flow, callback) {
        ...
        if (embedded.condition) {
            if (!embedded.condition(flow.currentStream, flow.context.getAll())) {
                callback();
        ...
        var collectionStream = utils.clone(flow.currentStream),
            startedAt,
            loggingLevel = 0
        ;
                ...
                function(stream) {
                    if (!flow.errored) {
                        ...
                        if (embedded.collection.aggregate) {
                            resolveOutput.call(self, embedded, stream, flow.currentStream);
                        // Handle specific collection output.
                            ...
                            for (var key in output) {
                                var argument = output[key],
                                    resolvedOutput = Array.isArray(stream) ? [] : {}
                                ;

                                for (var resolvedKey in stream) {
                ...
                function(asynchronousCollection, resolvedArguments, scope, callback) {
                    ...
                    self._sequencesContainer.get(sequenceId).forward(flow, function(error) {
                        flow.end(task, error, function(stream) {
                            ...
                            if (true === embedded.collection.aggregate) {
                                resolveOutput.call(self, embedded, stream, stream, itemTributary, loggingLevel);
                            }
...
var resolveOutput = function(embedded, context, stream, tributary, loggingLevel) {
    if (embedded.output) {
        var output = utils.clone(embedded.output),
            tributaryStream = this._referencesResolver.resolveSpecific(
                output,
                '@',
                context
            )
        ...
        if (embedded.merge) {
            tributaryStream = utils.merge(stream || {}, tributaryStream || {}, true);
        }
