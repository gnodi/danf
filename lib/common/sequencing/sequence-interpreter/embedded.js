        ...
        // Handle standard case.
            ...
            var currentStream = flow.currentStream[uniqueScope],
                tributary = flow.addTributary(uniqueScope, null, function() {
                    ...
                    resolveOutput.call(self, embedded, currentStream, flow.currentStream, tributary, loggingLevel);
    ...
    if (embedded.output) {
        ...
        for (var key in output) {
            ...
            if (
                ...
                'object' === typeof stream[key]
            ) {
                var isArgumentArray = Array.isArray(argument),
                    isStreamKeyArray = Array.isArray(stream[key])
                ;

                if (isArgumentArray && isStreamKeyArray) {
                    stream[key] = stream[key].concat(argument);
                } else if (!isArgumentArray && !isStreamKeyArray) {
                    stream[key] = utils.merge(
                        stream[key],
                        argument,
                        true
                    );
