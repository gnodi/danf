...
var interpretOperation = function(operation) {
    ...
    return function(flow, callback) {
        ...
        if (operation.condition) {
            if (!operation.condition(flow.currentStream, flow.context.getAll())) {
                callback();
        ...
        var scope = self._referencesResolver.resolve(operation.scope, flow.currentStream);
                ...
                function(asynchronousCollection, resolvedArguments, scope, callback) {
                    ...
                    method.__asyncApply(
                        ...
                        function(stream) {
                            ...
                            // Log errors.
                            if (flow.errored) {
                                for (var i = 0, length = flow.errors.length; i < length; i++) {
                                    self._logger.log(
                                        '<<red>><<bold>>x<</bold>> {0}'.format(flow.errors[i].message),
                                        3,
                                        3,
                                        tributary,
                                        loggingLevel,
