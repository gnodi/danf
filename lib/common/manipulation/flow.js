 ...
 * @interface {danf:manipulation.flow}
 */
Object.defineProperty(Flow.prototype, 'errored', {
    get: function() {
        var tributaryData = this._tributaries[this._currentTributary];

        return tributaryData.errors.length >= 1;
    }
});
 ...
 * @interface {danf:manipulation.flow}
 */
Object.defineProperty(Flow.prototype, 'errors', {
    get: function() {
        var tributaryData = this._tributaries[this._currentTributary],
            errors = []
        ;

        for (var i = 0; i < tributaryData.errors.length; i++) {
            errors[i] = tributaryData.errors[i];
        }
            ...
            if (tributaryData.errors.length !== 0) {
                ...
                if (tributaryData.catch) {
                    try {
                        var parentStream = parentTributaryData ? parentTributaryData.stream : this._mainStream,
                            returns = tributaryData.catch(tributaryData.errors, parentStream, tributaryData.stream)
                        ;

                        if (undefined !== returns) {
                            tributaryData.stream = returns;
                        }
                    } catch (error) {
                        errored = true;
                ...
                // Handle case where the triburaty does not handle its errors is the main one.
                    ...
                    if (1 === tributaryData.errors.length) {
                        ...
                        error = new Error('Errors happened in the flow "{0}".'.format(this.id));
                        ...
                        if (parentTributaryData) {
                            ...
                            if (null != scope) {
                                if ('.' !== scope) {
                                    ...
                                    if (Array.isArray(stream) && scopePath != parseInt(scopePaths[0], 10)) {
                                        stream = {};
                                    }
