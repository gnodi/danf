 ...
 * @interface {danf:manipulation.flow}
 */
Flow.prototype.addTributary = function(scope) {
    if (this._hasEnded) {
    ...
    if (scope) {
        if (this._stream && 'object' !== typeof this._stream) {
            throw new Error('Cannot set a scope for a tributary with a non object stream.')
        }

        if (this._stream && this._stream[scope]) {
            this._stream = this._stream[scope];
        } else {
            this._stream = null;
        }
    }
...
Flow.prototype.mergeTributary = function(tributary) {
    ...
    if (this._currentTributary === tributary) {
        ...
        if (hasChildrenEnded) {
            ...
            if (undefined !== tributaryData.parent) {
                ...
                if (tributaryData.scope) {
                    if (null == parentTributaryData.stream) {
                        parentTributaryData.stream = {};
                    }

                    parentTributaryData.stream[tributaryData.scope] = tributaryData.stream;
                } else {
