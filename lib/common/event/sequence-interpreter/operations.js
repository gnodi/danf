 ...
 */
function Operations(sequencesContainer, referenceResolver, servicesContainer, flowDriver, asynchronousCollectionsRegistry) {
 ...
 * @api public
 */
Object.defineProperty(Operations.prototype, 'asynchronousCollectionsRegistry', {
    set: function(asynchronousCollectionsRegistry) { this._asynchronousCollectionsRegistry = asynchronousCollectionsRegistry; }
});
    ...
    return function(flow, callback) {
        ...
        if (operation.collection) {
            var collection = operation.collection,
                asynchronousCollection = self._asynchronousCollectionsRegistry.get(collection.method)
            ;
            ...
            args.push(asynchronousCollection.wrapCallback(function(result) {console.log('-', result);
                if (undefined !== result) {console.log(0);
                    flow.setTributary(tributary);
            ...
            /*function(err) {
                ...
                if (null != error || null != result) {console.log('____', error,result);
                    flow.setTributary(tributary);
                    flow.currentStream = null == result ? error : result;
                    console.log('///', flow.currentStream, null == result ? error : result);
                    flow.mergeTributary(tributary);
                }
