...
var interpretOperation = function(operation) {
    ...
    return function(flow, callback) {
        ...
        // Handle standard case.
            ...
            self._logger.log(
                'Execute service <<bold>>{0}<</bold>> method <<bold>>{1}'.format(operation.service, operation.method),
                2,
                0,
                null,
                loggingLevel,
            ...
            service[operation.method].__asyncApply(
                ...
                function(stream) {
                    ...
                    if (null != scope) {
                        // Log output
                        self._logger.log(
                            '<<white>>< {0}: {1}'.format(scope, 'object' === typeof stream ? JSON.stringify(stream) : stream),
                            2,
                            1,
                            null,
                            loggingLevel
