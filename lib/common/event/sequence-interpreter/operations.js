    ...
    return function(flow, callback) {
        ...
        if (operation.collection) {
            ...
            args.push(function(value, key, callback) {
                ...
                var resolvedArguments = self.resolveArguments(operation.arguments, flow);
                ...
                var scope,
                    tributary = flow.addTributary(operation.scope, function(stream) {
                        /*if (Array.isArray(collection.input)) {
                            return Object.keys(stream).map(function(key) {
                                return stream[key];
                            });
                        }*/
                ...
                if (false === collection.aggregate && hasKey) {
                    scope = key;
                }

                service.__asyncFlow = flow;
console.log('/',operation.scope,scope);
