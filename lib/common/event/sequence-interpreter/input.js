 ...
 */
function Input(sequencesContainer, referenceResolver, dataResolver) {
 ...
 * @interface {danf:event.sequenceInterpreter}
 */
 Input.prototype.interpret = function(interpretation, definition) {
    if (definition.input) {
        interpretation.unshift({
            order: null,
            operations: [interpretInput.call(this, definition.input, definition.id)]
        });
    }
 ...
 */
var interpretInput = function(input, id) {
    var self = this;

    return function(flow, callback) {
        self._dataResolver.resolve(
            flow.stream,
            input,
