    ...
    return function(flow, callback) {
        ...
        if (embedded.collection) {
            ...
            self._collectionInterpreter.interpret(
                ...
                function(asynchronousCollection, resolvedArguments, scope, callback) {
                    ...
                    // Log execution
                    self._logger.log(
                        '<<yellow>>Sequence <<bold>>{0} on collection start'.format(sequenceId),
                        2,
                        0,
                        itemTributary,
                        loggingLevel,
        ...
        // Handle standard case.
                ...
                tributary = flow.addTributary(uniqueScope, null, function() {
                    ...
                    if (embedded.output) {
                        ...
                        for (var key in embedded.output) {
                            ...
                            // Log output
                            self._logger.log(
                                '<<white>>< {0}: {1}'.format(key, 'object' === typeof argument ? JSON.stringify(argument) : argument),
                                2,
                                1,
                                tributary,
                                loggingLevel
            ...
            for (var key in resolvedInput) {
                ...
                // Log input
                self._logger.log(
                    '<<white>>> {0}: {1}'.format(key, 'object' === typeof value ? JSON.stringify(value) : value),
                    2,
                    1,
                    tributary,
                    loggingLevel
