    ...
    get: function() {
        ...
        for (var i = 0; i < this._notifiers.length; i++) {
            (function(i) {
                ...
                var currentContract = notifierContract,
                    originalContract = utils.clone(notifierContract)
                ;

                for (var i = 0; i <= 9; i++) {
                    currentContract.children = {
                        type: 'embedded_object',
                        embed: utils.clone(originalContract)
                    };
                ...
                contract[notifier.name] = {
                    ...
                    validate: function(value) {
                        ...
                        for (var key in value) {
                            var itemDefinitions = mergeChildren.call(self, key, value[key], notifier);
    ...
    if (children) {
        for (var childName in children) {
            var child = children[name],
                childDefinitions = this.mergeChildren.call(this, childName, child, notifier);
            ;
