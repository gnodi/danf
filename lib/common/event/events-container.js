...
EventsContainer.defineDependency('_servicesContainer', 'danf:dependencyInjection.servicesContainer');
...
EventsContainer.defineDependency('_notifiers', 'danf:event.notifier_array');
 ...
 * @api public
 */
Object.defineProperty(EventsContainer.prototype, 'servicesContainer', {
    ...
    if (!reset) {
        for (var type in items) {
            for (var id in items[type]) {
                this.setDefinition(type, is, items[type][id]);
            }
        }
    }
...
EventsContainer.prototype.get = function(type, id) {
    ...
    if (!this.has(type, id)) {
        ...
        this._sequencesContainer.setDefinition(
            sequenceId,
            {children: event.sequences}
        );

        var notifier = getNotifier.call(this, type),
            sequence = this._sequencesContainer.get(sequenceId),
            eventObject = notifier.addListener(id, event, sequence)
        ;

        eventObject.notifier = notifier;

        this._events[type][id] = eventObject;
    }
