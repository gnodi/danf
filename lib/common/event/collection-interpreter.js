...
CollectionInterpreter.prototype.interpret = function(
    ...
    // Log start.
    self._logger.log(
        '<<green>>Collection <<bold>>{0}<</bold>> on <<bold>>{1}<</bold>> start'.format(collection.method, JSON.stringify(input)),
        2,
        0,
        tributary,
        loggingLevel,
    ...
    args.push(asynchronousCollection.wrapCallback(function(result) {
        ...
        self._logger.log(
            '<<magenta>>Collection <<bold>>{0}<</bold>>{1} end'.format(
                collection.method,
                null != result ? ' with <<bold>>{1}<</bold>>'.format(JSON.stringify(result)) : ''
            ),
            2,
            0,
            tributary,
